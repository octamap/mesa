...
export default function compileForOperator(dom: JSDOM, element: Element, variables: Variable[]) {
...
export function repeatElement(
    ...
    if (element.tagName.toLowerCase() == "template") {
        if ("content" in element) {
            elementsToClone = Array.from((element.content as any).children)
        }
    }

    items.forEach((dataItem, i) => {

        let clones = elementsToClone.map(x => x.cloneNode(true) as Element)
...
export function compileVariables(
    ...
    return code.replaceAll(regex, (_match, capture: string) => {
